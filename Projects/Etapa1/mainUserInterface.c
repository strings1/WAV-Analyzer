		...
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_COMMIT:
			...
			strcat(ConcatenatedPath,fileName);
			sprintf(filePath, ConcatenatedPath); //generare nume fisier
			GetCtrlDisplayBitmap(panel, ACQ_PANEL_SPECTRU_GRAPH_FILTERE, 1, &bitmapID);
			SaveBitmapToJPEGFile(bitmapID, filePath, JPEG_PROGRESSIVE, 100);
			break;
	}
	return 0;
...
int CVICALLBACK onSnapGraphFilteredFreqButton (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
		switch (event)
	{
		case EVENT_COMMIT:
			...
			GetCtrlVal(panel,ACQ_PANEL_FILENAME_GRPH_STRIN_3,fileName);
			char ConcatenatedPath[128];
			strcpy(ConcatenatedPath,FILE_PATH);
			strcat(ConcatenatedPath,fileName);
			sprintf(filePath, ConcatenatedPath); //generare nume fisier
			GetCtrlDisplayBitmap(panel, ACQ_PANEL_DSAMPLED_GRAPH_FILTER, 1, &bitmapID);
			SaveBitmapToJPEGFile(bitmapID, filePath, JPEG_PROGRESSIVE, 100);
			break;
	}
	return 0;
